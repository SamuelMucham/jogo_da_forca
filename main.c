...
void jogar() {
    ...
    // Abrir arquivo palavras.fc e posicionar no registro sorteado
    arq = abrirArq("palavras.fc", "r");
    fseek(arq, sizeof(guarda) * palavraso, SEEK_SET);
    fread(&guarda, sizeof(guarda), 1, arq);
    arq = fecharArq(arq);

    // Abrir arquivo dicas.fc e buscar a dica referente à palavra sorteada
    arq = abrirArq("dicas.fc", "r");
    fseek(arq, sizeof(gua) * guarda.idDica, SEEK_SET);
    fread(&gua, sizeof(gua), 1, arq);
    arq = fecharArq(arq);

    // Normalizar acentos para letras maiúsculas simples (A, E, I, O, U, etc.)
    for (i = 0; i < (int)strlen(guarda.palavra); i++) {
    ...
    // Inicializar palavra escondida (com '_' e espaços ou hífens visíveis)
    for (i = 0; i < (int)strlen(palavraEdit); i++) {
        if (palavraEdit[i] == ' ') {
            esconde[i] = ' ';
            ...
            esconde[i] = '_';
            tamanho++;  // Conta somente letras "escondidas"
        }
    }
    esconde[i] = '\0';

    j = 0;  // Índice para letras digitadas
    ...
    while (fim == false) {
        ...
        printf("\n\t\t\t");
        for (i = 0; i < (int)strlen(esconde); i++) {
            printf(" %c ", esconde[i]);
        }

        printf("\n\tLETRAS DIGITADAS:");
        for (i = 0; i < j; i++) {  // só imprime as letras já digitadas
            printf(" %c ", letDig[i]);
        }
        ...
        // Limpa buffer de entrada corretamente para evitar problemas
        fflush(stdin); // OBS: se possível substitua por função segura, ver nota abaixo
        scanf(" %c", &letra); // espaço antes de %c para ignorar '\n' pendente

        // Converter para maiúscula, cuidado com caracteres especiais
        if (letra <= 'z' && letra >= 'a') {
            letra = toupper(letra);
        } else if (letra == 135) { // ç
            letra = 'C';
        ...
        // Verifica se letra já foi digitada
        achou = false;
        for (i = 0; i < j; i++) {
            if (letra == letDig[i]) {
                achou = true;
        ...
        // Se letra nova, processa
        if (achou == false) {
            letDig[j] = letra;
            letDig[j + 1] = '\0';
            j++;

            // procLetra deve retornar true se a letra NÃO EXISTE na palavra
            // ou seja, erro ao tentar chutar uma letra não presente
            ...
            if (qtdErro >= 6) {
                fim = true;
                ganhou = false;
            } else if (strcmp(palavraEdit, esconde) == 0) { // compara com palavraEdit, não com palavra original
                ganhou = true;
                fim = true;
            }
        }
